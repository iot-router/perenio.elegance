...
function authenticator(validator, accs)
	...
	-- require "logging".info { authenticator = http.formvalue("auth", true) } 
...
function rpc_auth()
	...
	server.challenge = function(user, pass)
		...
		-- require"logging".info {session_list = util.ubus("session", "list")}
		...
		-- require"logging".info {login=login}
	...
	server.login = function(...)
		...
		-- require"logging".info{challenge=challenge}
		
		if challenge then
			
			http.header("Set-Cookie", 'web_auth=%s; path=%s; sameSite=none; secure' %{
				challenge.sid,
				http.getenv("SCRIPT_NAME")
			})
			
			return { sid=challenge.sid, token=challenge.token }
		end
